...
# 교사용 로그인 후 출결체크 방식 선택하는 화면 
class TeacherSelectAtd(QMainWindow):
    def __init__(self):
        super(TeacherSelectAtd, self).__init__()
        self.initUI()  # UI 초기화 함수 호출
...
class TeacherEachAtdCheck(QMainWindow):
    ...
    # 교사 로그인 했을 때 아이디를 가져와서 그 아이디에 맞는 반 조회하는 법
    def SGrade(self):
        self.t_id.T_ID
        # db연결
        # conn = oci.connect(f'{username}/{password}@{host}:{port}/{sid}')
...
if __name__ == '__main__':
    ...
    win = TeacherSelectAtd()  # MainWindow 객체 생성
    win.show()  # 메인 윈도우 표시
    app.exec_()  # 이벤트 루프 실행